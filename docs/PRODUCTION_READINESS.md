...
Last updated: 2026-08-09 (V8). This document is the single source of truth for
...
|---|---|---|---|
| Unit | `npm run test` | 79 | Pure logic, no DB access at import time |
| Integration | `npm run test:integration` | 96 | Full stack against ephemeral throwaway databases, sequential execution |
...
npm run test              # unit tests (79 tests, pure logic, no DB required)
npm run test:integration  # integration tests (96 tests, full stack against ephemeral DBs)
npm run test:concurrency  # concurrency tests (4 tests, N-way parallel against live stack)
npm run test:all          # all 179 tests
